...
var ValidLinks []string // links in the website as <a href="google.com">Google<>

func ExtractLinks(urlString string) ([]string, error) {
	...
	if resp.StatusCode != http.StatusOK {
		return nil, fmt.Errorf("Link not ok!", resp.Status)
	}

	doc, err := html.Parse(resp.Body)
	if err != nil {
		return nil, fmt.Errorf("Error parsing the html document!", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("Error parsing the link!", err)
	}

	var traverse func(*html.Node)
	traverse = func(n *html.Node) {
		if n.Type == n.ElementNode && n.Data == "a" {
			if  {
